...
# --- 修改：从环境变量获取路径，而不是硬编码 ---
# 默认路径（如果环境变量未设置，作为一个 fallback）
DEFAULT_CONFIG_PATH = "/data2/shaos/labs/mllms_know_94.1_head_select/head_analysisi/head_analysis_results/All_Samples_top_40_heads_by_ratio_L0_to_L27.jsonl"

# 优先读取环境变量 HEAD_CONFIG_PATH
HEAD_CONFIG_PATH = os.environ.get("HEAD_CONFIG_PATH", DEFAULT_CONFIG_PATH)

TARGET_HEADS_MAPPING = {}

if os.path.exists(HEAD_CONFIG_PATH):
    print(f"Loading head config from {HEAD_CONFIG_PATH}...")
    with open(HEAD_CONFIG_PATH, 'r') as f:
        for line in f:
            try:
                ...
                l_idx = data['Layer']
                h_idx = data['Head']
                if l_idx not in TARGET_HEADS_MAPPING:
                    TARGET_HEADS_MAPPING[l_idx] = []
                TARGET_HEADS_MAPPING[l_idx].append(h_idx)
                print(f"Added head {h_idx} to layer {l_idx}")
            except Exception as e:
                print(f"Error processing line: {line}. Error: {e}")
else:
    print(f"Warning: Config file not found at {HEAD_CONFIG_PATH}")

if TARGET_HEADS_MAPPING:
    SELECT_LAYER = list(TARGET_HEADS_MAPPING.keys())
    print(f"Selected layers: {SELECT_LAYER}")
else:
    SELECT_LAYER = [15]
    print("No head config found, using default layer 15.")
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if output_attentions:
            def chunked_attention(module, query_states, key_states, head_dim, target_indices=None, chunk_size=512, attention_mask=None):
                ...
                """
                Modified to compute attention only on specific heads defined in global TARGET_HEADS_MAPPING.
                """
                ...
                # 获取当前层需要计算的 heads 列表
                # 如果当前层不在 mapping 中，默认不计算或者计算全部（根据你的需求）
                # 这里假设：如果在 SELECT_LAYER 中但不在 mapping 中，则计算全部；如果在 mapping 中，则只计算指定的。
                current_layer_idx = module.layer_idx
                if current_layer_idx in TARGET_HEADS_MAPPING:
                    target_heads = TARGET_HEADS_MAPPING[current_layer_idx]
                else:
                    target_heads = range(H) # Fallback to all heads if no specific config for this layer

                # 有效 Head 数量，用于归一化
                num_active_heads = len(target_heads)
                ...
                for i in range(0, num_targets, chunk_size):
                    ...
                    B, H, chunk_q_len, _ = q_chunk.shape
                    
                    # 累加器，用于存放所选 Head 的 Attention Sum
                    ...
                    for h in target_heads:
                        ...
                        attn_chunk_h = torch.matmul(q_chunk_h, key_states_h.transpose(2, 3)) * scale
                        
                        # 应用 mask（如果存在）
                        if attention_mask is not None:
                            causal_mask = attention_mask.index_select(dim=2, index=current_indices)
                    ...
                    # ✅ 转到 CPU，求均值
                    # 注意：这里除以 num_active_heads 而不是 H
                    attn_chunk_cpu = (attn_chunk / num_active_heads).detach().cpu().float().numpy()  # [B, 1, chunk_q, K_LEN]
                    del attn_chunk
